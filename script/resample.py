    ...
    for variable in ['QPE', 'QPF']:  # variable name to resample in source file
          ...
          for t in range(len(time)):
            ...
            for hour in range(6):
              ...
              year = str(datetime.datetime.fromtimestamp(time[t] * 60 + hour *
                                                     3600).year)
              month = str(datetime.datetime.fromtimestamp(time[t] * 60 + hour *
                                                     3600).month)
              ...
              if  hr==0:
                  endtime = string.replace(
                      datetime.datetime.fromtimestamp(time[t] * 60 + hour * 3600 - 86400).strftime(
                      '%d%b%Y:%H%M'), '0000', '2400')
                  print "starttime=" + starttime + "endtime="+endtime
              else:
                  endtime = datetime.datetime.fromtimestamp(time[t] * 60 + hour * 3600).strftime(
                      '%d%b%Y:%H%M')                  
