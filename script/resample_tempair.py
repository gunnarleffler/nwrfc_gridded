...
'''
Execute on command line passing QTE and QTF netcdf grids for 1st and second arguemnt
...
for variable in ['TFMX', 'QTE']:  # variable name to resample in source file
    ...
    for hour in range(6):
      ...
      endtime = string.replace(
          datetime.datetime.fromtimestamp(time[t] * 60 + hour * 3600).strftime(
              '%d%b%Y:%H%M'), '0000', '2400')
      dss_path = "/SHG/" + project + "/TEMP-AIR/" + starttime + "/" + endtime + "/NETCDF/"
