...
def temp(df_daily, df_disagg, t_t_min, t_t_max, ts, t_begin, t_end):
  ...
  time = np.array(
      list(next(it) for it in itertools.cycle([iter(t_t_min), iter(t_t_max)])))
  temp = np.array(
      list(
          next(it)
          for it in itertools.cycle(
...
for index, row in basin_data.iterrows():
  ...
  for variable in ['QTF', 'QTE']:  # variable name to resample in source file
    ...
    fr.close()
    #convert local time to GMT for each timestamp
    ...
    if variable == 'QTF':
      ...
      for t in range(2, len(time) - 2):
        if (datetime.datetime.fromtimestamp(time[t] * 60).hour == 10):
          tmin_resample[ind_mn, :, :] = pyresample.kd_tree.resample_gauss(
              origin_grid,
              TAMN[t],
              targ_def,
        ...
        if (datetime.datetime.fromtimestamp(time[t] * 60).hour == 21):
          tmax_resample[ind_mx, :, :] = pyresample.kd_tree.resample_gauss(
              origin_grid,
              TAMX[t],
              targ_def,
              radius_of_influence=source_res,
...
# and interpolate temperature
      ...
      days = pd.date_range(
          datetime.datetime.fromtimestamp(time[2] * 60).strftime('%Y%m%d'),
          datetime.datetime.fromtimestamp(time[len(time) - 3] *
                                          60).strftime('%Y%m%d'),
          freq='D')
      ...
      for i in range(len(tmin_resample[0, :, 0])):
        for j in range(len(tmax_resample[0, 0, :])):
          df_daily = {
              't_min': pd.Series(tmin_resample[:, i, j], index=days),
              't_max': pd.Series(tmax_resample[:, i, j], index=days)
          }
          dates_disagg = pd.date_range(
              df_daily['t_min'].index[0],
              df_daily['t_min'].index[-1] + pd.Timedelta('1 days'),
              freq='{}T'.format(Dt * 60))
          df_disagg = pd.DataFrame(index=dates_disagg)[1:41]
          hourlyts[:, i, j] = temp(df_daily, df_disagg, t_t_min, t_t_max, Dt *
                                   60, None, None)
